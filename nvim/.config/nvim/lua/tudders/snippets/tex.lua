...
local snippets = u.make({
	...
	fig = {
		...
		t({ "\\begin{figure}[" }),
		i(1, "!htpb"),
		t({ "]", "\t\\centering", "\t\\includegraphics[width=0.7\\textwidth]{" }),
		i(2, "image.jpg"),
		t({ "}", "\t\\caption{" }),
		i(3, "caption"),
		t({ "}\\label{fig:" }),
...
local autosnippets = {
	...
	s(
		{ trig = "pmat", desc = "pmatrix", wordTrig = false },
		{ t({ "\\begin{pmatrix}", "" }), i(0), t({ "", "\\end{pmatrix}" }) }
	),

	s(
		{ trig = "bmat", desc = "bmatrix", wordTrig = false },
	...
	s(
		{ trig = "ceil", desc = "ceil", wordTrig = false },
		{ t({ "\\left\\lceil " }), i(1), t({ " \\right\\rceil" }) },
		{ condition = tex_maths }
	),
	s(
		{ trig = "floor", desc = "floor", wordTrig = false },
		{ t({ "\\left\\lfloor " }), i(1), t({ " \\right\\rfloor" }) },
		{ condition = tex_maths }
	),
	s(
		{ trig = "sq", desc = "square root", wordTrig = false },
		{ t({ "\\sqrt{" }), i(1), t({ "}" }) },
		{ condition = tex_maths }
	),
	s(
		{ trig = "exp", desc = "exponential", wordTrig = false },
		{ t({ "\\exp{" }), i(1), t({ "}" }) },
		{ condition = tex_maths }
	),
	...
	s({ trig = "taylor", desc = "Taylor series" }, {
		t({ "\\sum_{" }),
		i(1),
		t({ "=1" }),
		t({ "}^{\\infty} c_" }),
